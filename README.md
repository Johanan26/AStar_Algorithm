...
**Project Title:** A* Pathfinding Visualisation in C++   

---

## Overview

This project implements the **A\*** algorithm in **modern C++** to find the shortest path between the start point and the end point on a grid containing obstacles. The system was designed using a modular and object-oriented structure so that the logic for the grid, algorithm, point handling, and display are separated into different files. In terms of the movement calculations I used Manhatten Distance to calculate where the algorithm can move and where it is restricted.
...
- To implement the **A\*** algorithm correctly
...
The program creates a matrix grid made up of cells (6,8). Some cells are walkable (0) and some are obstacles (1). A start point (S) and an end point (E) are chosen, and the A\* algorithm searches for the shortest route between them.
...
This maded Manhattan distance a suitable and efficient choice.
...
### `Grid`
The `Grid` class stores the grid layout, including blocked and free cells, as well as the start and end positions. It also provides functions for checking if a cell is valid and walkable.
...
A* was chosen due to its reputation as one of the most effective and widely recognised algorithms for shortest path problems where a heuristic is available. Unlike other approaches such as Dijkstra's algorithm, A* actively guides its search toward the goal, making it more efficient in many cases.

It was a good choice for this project because:
- it demonstrates clear algorithmic reasoning
- it is widely used in games and navigation systems
- it fits naturally with grid-based environments
...
The A\* algorithm begins at the starting point and keeps track of nodes to be explored. For each node, it calculates the movement cost from the start and the estimated cost to the goal.
...
-the heuristic is admissible and suitable for four-directional movement
...
### Code Example 1: Program Setup

Add a screenshot here showing the section of your `main.cpp` where the grid is created, the test case is run, and the algorithm is called.

**Add screenshot here:** `images/main-setup.png`  
**What it should show:**  
- grid creation  
...
Add a screenshot here showing functions from `Grid.cpp` or `Grid.h`.

**Add screenshot here:** `images/grid-logic.png`  
**What it should show:**  
- `isInside()`  
- `isWalkable()`  
...
Add a screenshot here showing the main section of `AStar.cpp`.

**Add screenshot here:** `images/astar-core.png`  
**What it should show:**  
- open list selection  
- neighbour checking  
- cost updates (`g`, `h`, `f`)  
...
**Suggested caption:**  
*Figure 3. Core A\* implementation showing cost calculation, node expansion, and path reconstruction.*

---

### Code Example 4: Output / Display

Add a screenshot here showing your display logic or terminal output code.

**Add screenshot here:** `images/display-output.png`  
**What it should show:**  
- rendering of grid  
- start/end labels  
- path symbols  
...
Add a screenshot here of the program running successfully in the terminal.

**Add screenshot here:** `images/program-run.png`  
**What it should show:**  
...
4. Build the A\* pathfinding logic
...
This project improved my understanding of both algorithmic problem-solving and code organisation in C++. Before completing the project, I understood the general idea of pathfinding, but implementing A\* in code made the importance of data structures, cost tracking, and parent reconstruction much clearer.
...
If I were to improve the project further, I would extend it by:
...
If I were doing the project again, I would probably introduce structured test cases earlier in development rather than mainly validating after the core implementation was working. That would make debugging even more efficient and strengthen the validation section further.
